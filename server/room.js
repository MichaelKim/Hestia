...
module.exports = {
  ...
  createRoom: function(host: Player) {
  ...
  joinRoom: function(roomId: number, player: Player) {
    player.room = roomId;
    player.role = 1;
  ...
  leaveRoom: function(player: Player) {
    ...
    for (var i = 0; i < roomPlayers.length; i++) {
      if (roomPlayers[i].id === player.id) {
        ...
        } else if (this.rooms[player.room].host === player.id) {
  ...
  roomExists: function(roomId: number) {
    return this.rooms[roomId] !== undefined;
  },

  getAppId: function(roomId: number) {
  ...
  setAppId: function(roomId: number, appId: number) {
